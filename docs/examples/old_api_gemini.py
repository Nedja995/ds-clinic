...
import config
import word_utils
from models import AnalysisReport  # Importujemo našu detaljnu šemu
...
##
#
def _load_document_from_file(filepath: str) -> genai.types.Part:
...
def analyze_docs(ai_task_description: str = config.AI_TASK_DESCRIPTION,
    ...
    '''
    This function orchestrates the document analysis process using the Gemini API.
    It connects to the Gemini client, loads documents, and attempts to analyze them.
    In case of high demand on the primary model, it retries with a fallback model.

    Args:
        ai_task_description (str): The description of the AI task to be performed.
        documents_filepaths (List[str]): A list of file paths to the documents to be analyzed.
...
def analyze_lab_result_docs(client: genai.Client,
    ...
    for filepath in documents_filepaths:
        doc = _load_document_from_file(filepath)
        if doc:
    ...
    generate_content_config = types.GenerateContentConfig(
        ...
        response_schema=AnalysisReport,  # OVO PRIMORAVA MODEL DA PRATI NAŠU PYDANTIC STRUKTURU!
    ...
    try:
        ...
        print(f"\n----- |GEMINI| Analysis: Response JSON created successfully.")
    except json.JSONDecodeError as e:
        print(f"\n---------- |GEMINI| - ERROR decoding JSON:\n{e}.")
        responseDict = {}
